...
void update(nwm::De &env, nwm::Base &test, application::app &apps) {
  env.window = XCreateSimpleWindow(
      test.display, test.root, POSITION_X, POSITION_Y,
      WIDTH(test.display, SCREEN_NUMBER), HEIGHT(test.display, SCREEN_NUMBER),
      BORDER, BlackPixel(test.display, test.screen),
      BlackPixel(test.display, test.screen));
